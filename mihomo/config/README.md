...
本目录包含 4 个采用不同设计哲学和控制粒度的 Mihomo 配置文件，从追求极致掌控的精细化配置到基于算法的自动化选择，满足您在不同场景下的策略偏好：

| 配置文件 | 版本 | 复杂度 | 适用场景 | 文件大小 |
|---------|------|--------|---------|----------|
| `MihomoPro.yaml` | v26.1.3 | ⭐⭐⭐⭐⭐ | **「经典全面掌控」** 经典测速与多策略精细分流 | 22.6 KB |
| `OneSmartPro.yaml` | v26.1.3 | ⭐⭐⭐⭐ | **「智能全面掌控」** 智能自适应与复杂拓扑中转 | 20.1 KB |
| `OneSmart.yaml` | v26.1.3 | ⭐⭐⭐ | **「智能极简」** 智能自适应与轻量极简自治 | 12.5 KB |
| `OneTouch.yaml` | v26.1.3 | ⭐⭐ | **「经典极简」** 经典测速与极致纯净轻量 | 12.4 KB |
...
✅ 追求对不同网络流量进行精细、独立控制的手动掌控  
✅ 需要内置广告拦截、追踪保护和海外 QUIC 强制阻断以提升准确性  
✅ 对不同流媒体、社交网络和 AI 服务有明确且独立的路由分配策略  
✅ 需要下载大流量自动直连，不侵占代理通道带宽
...
✅ 倾向于让算法自动寻优决策，减少频繁手动切换策略组的精力开销  
✅ 持有多家机场订阅，需要基于拓扑中转、多端口监听和权重智能分配的高级网络架构  
✅ 需要通过多监听器暴露分地区代理端口（如 Mixed 多端口），实现客户端的分区精细控制  
✅ 希望利用 LightGBM 机器学习算法（基于 policy-priority）自适应分配机场优先级
...
✅ 喜爱 LightGBM 智能节点寻优，同时追求极致清爽的规则面和高解析速度  
✅ 崇尚“高效自治”的设计哲学，希望配置文件结构简单、易于长期维护  
✅ 习惯在没有广告/追踪拦截规则包干扰的纯粹网络环境下冲浪  
✅ 希望智能自适应选择节点，省去手动测速和选择的烦恼
...
✅ 崇尚传统物理测速（URL-Test），希望以最直观的延迟结果自动漂移节点  
✅ 追求零附加规则的纯净状态，规避复杂的广告过滤污染，专注于最极致的吞吐响应  
✅ 流媒体重度用户，需要对 YouTube、Netflix、Spotify 等主流流媒体平台进行精细分流控制  
✅ 追求“极轻极简，测速即连”的低负载、高响应体验
...
- 免认证网段：127.0.0.1/8、::1/128（仅默认豁免本机回环，保障局域网安全）
...
根据您的**网络结构偏好与控制粒度需求**进行选择：

```
🎛️ 「经典全面掌控」手动全维精细掌控与策略组     ➔ MihomoPro.yaml
🕸️ 「智能全面掌控」算法智能自适应与链式中转       ➔ OneSmartPro.yaml
🧠 「智能极简」算法智能自适应与极简自治           ➔ OneSmart.yaml
⚡ 「经典极简」经典测速与极致纯净极简骨架         ➔ OneTouch.yaml
```
...
| **规则集数量** | 30+ | 25+ | 15+ | 18+ |
| **应用分流细分** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ |
| **控制深度** | 极深（全维度细分） | 深（多维分流） | 轻量（聚焦核心） | 极轻（最简骨架） |
| **核心策略哲学** | 「经典全面掌控」手动全维精细管理 | 「智能全面掌控」智能自适应与复杂调度 | 「智能极简」清爽高效与算法自治 | 「经典极简」经典测速与极致纯净极轻 |
...
- ✨ **新增**: `empty-fallback: REJECT` 之前无节点时, 会自动使用 COMPATIBLE 即直连。现在无节点就 REJECT, 防止侧漏 🐶

### v26.1.2 (2026-05-25)

- 🛡️ **安全**: `allow-lan` 默认设为关闭 (`false`)，避免在未知的公共网络下默认暴露端口，交由客户端动态 Patch
- 🛡️ **安全**: `skip-auth-prefixes` 清理了硬编码的局域网私有网段，仅默认豁免本机物理回环 `127.0.0.1/8` 与 `::1/128`，大幅提升代理认证安全性
- 📝 **更新**: 全系列 4 套配置文件版本号统一升级至 v26.1.2

### v26.1.3 (2026-01-30)

- ♻️ **移除**: 移除 `global-client-fingerprint: chrome` 配置，同步 Mihomo 官方移除
- ♻️ **优化**: ChinaIP 规则移除 `no-resolve` 参数，确保国内域名正确解析 IP 后匹配

### v26.1.0 (2026-01-09)

- ♻️ **优化**: QUIC 阻止规则改为仅阻止海外 QUIC，保留国内 QUIC 提升国内访问速度
- ♻️ **优化**: 移除前置加速 URL（git.imee.me），改用 GitHub 原始链接(CF每天的 10W 请求完全顶不住了)
- 📝 **更新**: 所有配置文件版本号统一升级至 v26.1.0
